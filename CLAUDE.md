...
## アーキテクチャ上の注意点
- **DPI awareness は app.manifest で宣言する（v1.9.2 で復元）**: `<dpiAware>true/pm</dpiAware>` + `<dpiAwareness>PerMonitorV2</dpiAwareness>`（2016 名前空間）の **2 要素形式が必須**（`<dpiAware>PerMonitorV2</dpiAware>` は無効値で USER32 が 0xC0000409 クラッシュする既知の罠）。csproj の `ApplicationHighDpiMode` は **WinForms 専用**（`ApplicationConfiguration.Initialize()` を呼ぶアプリのみ有効）で WPF エントリポイントでは効かない。v1.1.0 で WFAC010 警告解消のため誤ってそちらへ移行した結果、プロセスが System DPI Aware に落ち、混在 DPI 環境で読み取り系 API（`GetWindowRect`/`GetCursorPos`/`Screen.Bounds`）が仮想化座標（例: システム 150% のとき 100% モニタ上は×1.5）を返して座標系が破綻していた（v1.9.0 の前面ウィンドウ中央ジャンプが 1.5 倍先へ飛ぶ・画面端で止まる症状の根本原因）。WFAC010 は csproj の `NoWarn` で理由コメント付き抑止。実効状態は起動時 debug.log の `ProcessDpiAwareness:` 行（期待値 `Process_Per_Monitor_DPI_Aware`）で常時確認できる
...
- **MouseButtonType enum**: Left=0, Right=1, Middle=2, XButton1=3, XButton2=4, MiddleLeftChord=5, MiddleRightChord=6, MiddleDoubleClick=7, MiddleTripleClick=8, WheelUp=9, WheelDown=10, MouseWheel=11（末尾追加で後方互換性維持）。WheelUp/WheelDown は v1.5.2 で追加したが v1.6.1 で UI からは除外（旧 settings.json 互換のため enum 値は残す）。新規割当は MouseWheel（上下方向中立の統合トリガー）を使う
- **マウスホイールトリガー (v1.6.1+)**: `MouseHookService` の WM_MOUSEWHEEL 処理は delta 符号から `WheelDirection.Up/Down` を算出し `MouseHookEventArgs.Direction` に格納する。マッチング判定 `MatchWheelShortcut` は新 `MouseWheel`（方向中立、`AreModifiersHeld` のみ判定）と旧 `WheelUp/WheelDown`（方向別、`IsShortcutMatch` で従来通り）の両方を受け付ける。`MainWindow` の Navigate 系ハンドラは `e.Direction == Up` のとき `GetPrev` 系、それ以外は `GetNext` 系を呼ぶ。Save/SaveB/DisplayDelete は方向非依存（どちらでも発火）
- **軌跡/収縮円オーバーレイのシングルトン再利用 (v1.6.1+)**: `OverlayService._trailOverlay` を `MainWindow.OnSourceInitialized` 末尾で 1 枚だけプリアロケートし `Show()`+`CoverVirtualScreen()`+`TrackDisplaySettingsChanges()` 済みで常駐させる。`ShowTrail` / `ShowShrinkCircle` / `ShowClearAllShrinkCircles` は共有オーバーレイの Canvas に要素を追加し、アニメ完了で個別要素のみ Remove する（ウィンドウは閉じない）。3画面マルチモニタ + WS_EX_LAYERED HWND の生成コストを 1 回で済ませることで、ジャンプ後のラグ・砂時計カーソルを解消する。`WS_EX_NOACTIVATE` 併用でアクティベート同期も抑止。削除モードのマーカーオーバーレイ (`_markerOverlay`) は半透明背景つき別系統で従来通り都度生成
...
6. 生成された `Releases/` 配下の以下 6 ファイルを GitHub Releases にアップロード（タグは `v<VERSION>` 形式）:
   - `CursorJump-win-Setup.exe`
   - `CursorJump-<VERSION>-full.nupkg`
   - `CursorJump-<VERSION>-delta.nupkg`
   - `RELEASES`（旧形式インデックス）
   - `releases.win.json`（**新形式インデックス。Velopack 0.0.1298+ はこちらを優先参照**）
   - `assets.win.json`
7. GitHub Release Body にも同 Markdown を貼る（人間が Releases ページで読む用。Velopack のダイアログ表示には影響しない＝nuspec の埋め込み値が使われる）

> **重要**:
> - 手順 5 で `--releaseNotes` を省略すると `<releaseNotes>` が空のまま埋め込まれ、アプリ内アップデートダイアログのリリースノート欄が `-`（空）表示になる。GitHub Release Body をいくら書いても直らない（Velopack は Body を参照しない）。v1.4.2 でこの事故が発生したため v1.4.3 で恒久対策。
> - 手順 6 で `releases.win.json` のアップロードを忘れると、Velopack のクライアントが新版を検出できず、旧 `RELEASES` ベースの古い結果（前回バージョン）が返ることがある。v1.4.3 公開直後にこの事故が発生したため、新形式インデックスも必ず含めること。
